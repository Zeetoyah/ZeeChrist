...
*Runtime :* ${aqul.runtime(anu)}`
			...
			case 'add':
				...
				break
				case 'glitch':
				if (!arg) return aqul.reply(from, `Penggunaan ${prefix}glitch teks1|teks2`, qul)
				argz = arg.split("|")
				if (!argz) return aqul.reply(from, `Penggunaan ${prefix}glitch teks1|teks2`, qul)
				
				axios.get(`https://onlydevcity.herokuapp.com/api/textmaker?text=${argz[0]}&text2=${argz[1]}&theme=glitch&apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				...
				break
				//fitur tambahan by :Zee
						case 'hentai':
		
				axios.get(`https://onlydevcity.herokuapp.com/api/hentai?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				...
				axios.get(`https://onlydevcity.herokuapp.com/api/nsfw/nekonime?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				...
				break
				case 'neko':
				axios.get(`https://onlydevcity.herokuapp.com/api/nsfw/neko2?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.neko))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				})
				break
				case 'blackpink':
				axios.get(`https://onlydevcity.herokuapp.com/api/randomimage/blackpink?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.result))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				})
	...
	case 'asupan':
				...
				axios.get(`https://onlydevcity.herokuapp.com/api/asupan?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
     ...
     case 'amel':
				...
				axios.get(`https://onlydevcity.herokuapp.com/api/asupanamel?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
				...
				axios.get(`https://onlydevcity.herokuapp.com/api/asupanuna?apikey=Zeetoyah`)
				.then((res) => aqul.sendMediaURL(from, res.data.result.url))
				.catch((err) => {
					console.log(err)
					aqul.reply(from, mess.error.api, qul)
...
283722
MILF
318321 302842 301617
216805 155244 240263
149495 133637 149197
202158 304110 154797
288828 193555 186169

–Tsunade
333709 334512 324199
329212
–TsukoyomiTsukasa
334544
–RiasGremony
107674 216758`
				aqul.sendFakeStatusWithImg(from, fs.readFileSync(`./media/hentai.jpeg`), textnya, fake)
				break
				
                  
			default:
				if (chats.startsWith('>')){
					console.log(color('[Zee-Bot]'), color(moment(qul.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`Eval brooo`))
                	return aqul.reply(from, JSON.stringify(eval(chats.slice(2)), null, 2), qul)
				}
				break
        }
    } catch (err) {
        console.log(color('[ZEE]', 'red'), err)
    }
})
